...
public class FileServer {
	...
	public boolean isLegalAbsolutePath(String path) {
		//rules out /../ occurrences
		if(path.contains("/../")){
			return false;
		}
		//looks for the pattern we want
		Pattern p = Pattern.compile("^\\/([A-z]|[a-z]|[0-9]|[\\.\\_\\-\\~\\/])*?");
		Matcher m = p.matcher(path);
		//true if the entire path matches desired pattern
		return m.lookingAt();
	}
	...
	public void runSingleClient(ServerSocket serverSock, PrintStream sysErr) {
		...
		while(!exit){
			try {
				//initiallize all writers, readers and client
				user = serverSock.accept();
				...
				if(!m.group(1).toUpperCase().equals("GET")){
					bw.write(RESP_BADMETHOD);
					bw.flush();
					continue;
				}
				/*
				if the pathname is exactly {@value #PATH_EXIT} (case sensitive), send
				{@link #RESP_EXIT} to the client and close the connection and exit
				 */
				...
				/*
				if the path is a legal path but the file does not exist or cannot be read,
	 * 			{@link #RESP_NOTFOUND}, followed by the path itself, to the client and close the connection.
				 */
				else if(!isLegalAbsolutePath(path)){
					bw.write(RESP_BADPATH + " " + path);
					bw.flush();
					continue;
				}
				/*
				send {@link #RESP_OK} to the client, then open the file
	 * 			and send its contents to the client. Finally, close the connection
				 */
				file = new File(path);
				if(Files.exists(file.toPath())){
						try {
							...
							bw.write(RESP_OK);
							bw.flush();
							user.getOutputStream().write(fileContent);

						}catch(IOException e){
							e.printStackTrace();
						}
					/*if the path is a legal path but the file does not exist send
	 				{@link #RESP_NOTFOUND}, followed by the path itself, to the client and close the connection.
					 */
				}else if(!file.exists()){
					bw.write(RESP_NOTFOUND + " " + path);
					bw.flush();
				}
			} catch (IOException e) {
